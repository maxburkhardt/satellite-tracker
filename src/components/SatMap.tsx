...
class SatMap extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      ...
      map: new OlMap({
        layers: [
          new OlLayerTile(layerTileOptions)
        ],
        view: new OlView({
  ...
  generateMarkers(sat:SatellitePosition) {
    const mapFeature = new OlFeature({
      geometry: new OlPoint(
        fromLonLat([sat.longitude, sat.latitude])
      ),
      name: sat.name
    });
  ...
  componentDidUpdate() {
    ...
    if (this.props.userLocation.latitude !== this.state.groundStationLocation.latitude ||
         this.props.userLocation.longitude !== this.state.groundStationLocation.longitude) {
      this.state.map.getView().animate({zoom: 4}, {center: fromLonLat([this.props.userLocation.longitude, this.props.userLocation.latitude])});
