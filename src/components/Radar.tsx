...
class Radar extends React.Component<Props, State> {
  ...
  generateSatIcon(sat: SatellitePosition, xOffset: number, yOffset: number, radius: number): JSX.Element {
    const xPos = radius * Math.cos(sat.azimuth - Math.PI / 2) + xOffset;
    const yPos = radius * Math.sin(sat.azimuth - Math.PI / 2) + yOffset;
    const iconSize = 10;
    return (
      <React.Fragment>
        <rect x={xPos - (iconSize / 2)} y={yPos - (iconSize / 2)} height={iconSize} width={iconSize} />
        <text x={xPos + iconSize} y={yPos + 5}>{sat.name}</text>
      </React.Fragment>
    );
  }
  ...
  render() {
    ...
    const satellites = this.getOverhead().map((sat) => this.generateSatIcon(sat, xCenter, yCenter, radius));
    return (
      <div className="radarDiv">
        ...
        <svg className="radarSvg" xmlns="http://www.w3.org/2000/svg">
          <circle cx={xCenter} cy={yCenter} r={radius} fill="none" stroke={uiColor} />
          <circle cx={xCenter} cy={yCenter} r={radius / 2} fill="none" stroke={uiColor} />
          <line x1={0} y1={yCenter} x2={this.state.width} y2={yCenter} stroke={uiColor} />
          <line x1={xCenter} y1={0} x2={xCenter} y2={this.state.height} stroke={uiColor} />
          {satellites}
        </svg>
      </div>
    );
