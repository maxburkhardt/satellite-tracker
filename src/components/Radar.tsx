...
class Radar extends React.Component<Props> {
  ...
  clearRadar() {
    if (this.satellitesOnRadar.current) {
      const ctx = this.satellitesOnRadar.current.getContext('2d');
      if (ctx) {
        ctx.clearRect(0, 0, this.satellitesOnRadar.current.width, this.satellitesOnRadar.current.height);
      }
    }
  }
  ...
  render() {
    return (
      <div className="radarDiv">
        <canvas className="radarCanvas" ref="radarUi" width={300} height={300}></canvas>
        <canvas className="satellitesCanvas" ref="satellitesOnRadar" width={300} height={300}></canvas>
      </div>
    );
  }
}
